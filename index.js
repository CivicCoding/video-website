    ...
    $(window).scroll(function () {
        ...
        if(top>breakPoint-300){
            ...
            $(".news-list").each(function (index) {
                $(this).delay(index*400).animation({top:0},500)
            })
...
//文字波动效果
{
    $(".content-name p:first-child").each(function () {
        ...
        let obj=this;
        let span=[];
        Array.form(content).forEach(function (word, index) {
            $("<span>").html(word).css({
                position:"relative",
                left:0,
                top:0
        ...
        span.forEach(function (ele,index) {
            ele.on("transition","all .3s")
            ele.css("top" ,0);
            ele.css("transitioned",function () {
